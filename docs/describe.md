...
**Гравитационная сила**:
$$\vec{F}_{g} = - \frac{\mu \cdot m}{|\vec{r}|^3} \vec{r}$$

**Сила пружины** (закон Гука): 

$$ \vec{F}_{spring} = k \cdot (L_0 - d) \cdot \frac{\vec{r}_{rel}}{d} $$

где $k$ — жёсткость, $L_0$ — длина в недеформированном состоянии, $d$ — текущая длина. Сила обнуляется, если $d >= L_0$ или если $d$ достигает механического упора ($L_{final}$).
