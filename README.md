...
Для работы проекта необходим Python 3.8+ (согласно `pyproject.toml`). Все зависимости указаны в файле `pyproject.toml`.
...
Для установки зависимостей для разработки (включая `pytest` для тестов):
```bash
pip install ".[dev]"
```
...
*   `simulation_plots_relative.png` — графики относительного положения и скоростей координат положения КА и РН по осям X, Y, Z.
